	...
	void Update () {
        ...
        if (target != null){
            ...
            if (XMinEnabled && XMaxEnabled)
                targetPos.x = Mathf.Clamp(target.transform.position.x, XMinValue, XMaxValue);
            else if (XMinEnabled)
                targetPos.x = Mathf.Clamp(target.transform.position.x, XMinValue, target.transform.position.x);
            else if (XMaxEnabled)
                targetPos.x = Mathf.Clamp(target.transform.position.x, target.transform.position.x, XMaxValue);
        }
	}
}
